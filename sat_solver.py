...
def solve_sat_for_init_hands(public_state_array, num_solutions):
    ...
    solutions = solver.enum_models(
        assumptions=positive_literals+negative_literals)
    # High number to ensure sufficient randomness
    sols = list(islice(solutions, 2000))
...
def top_k_likely_hands(ruleset, current_state, k, policy_model, init_hands_to_sample=200, epsilon=1e-4):
    ...
    init_states = [current_state.recover_init_state(
        initial_hands) for initial_hands in top_candidates]
    ...
    for init_state in init_states:
        ...
        # Every init hand has equal number of actions to evaluate
        assert actions_per_init_state == added_states
    ...
    # Run NN
    nn_states = [state.state_for_player(
        state.active_player).state_for_nn[None, ...] for state in all_states]
    ...
    log_probs_of_taken_actions = log_probabilities[np.arange(
        len(all_actions)), np.array(all_actions)]
    log_probs_by_init_state = log_probs_of_taken_actions.reshape(
        (num_sampled, actions_per_init_state))
